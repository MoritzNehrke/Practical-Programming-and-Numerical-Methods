...
vector gradient_fd(std::function<double(const vector&)> phi, vector x){
	int n = x.size();
	double phix = phi(x);
	vector gphi(n);
	for(int i=0; i<n; i++){
...
vector gradient_cd(std::function<double(const vector&)> phi, vector x){
	int n = x.size();
	vector gphi(n);
	for(int i=0; i<n; i++){
		double dxi = (1+std::abs(x[i]))*std::pow(2.0,-52.0/3);
		pp::vector xp = x;
		pp::vector xm = x;
		xp[i] += dxi;
		xm[i] -= dxi;
		gphi[i] = (phi(xp)-phi(xm))/2.0/dxi;
	}
	return gphi;
}

matrix hessian_fd(std::function<double(const vector&)> phi, vector x){
	int n = x.size();
	matrix H(n,n);
	vector gphix = gradient_fd(phi,x);
	for(int j=0;j<n;j++){
		double dxj = (1+std::abs(x[j]))*std::pow(2.0,-13);
		x[j] += dxj;
		vector dgphi = gradient_fd(phi,x) - gphix;
		for(int i=0;i<n;i++) H(i,j) = dgphi[i]/dxj;
		x[j] -= dxj;
	}
	return H;
}

matrix hessian_cd(std::function<double(const vector&)> phi, vector x){
	int n = x.size();
	matrix H(n,n);
	double phix = phi(x);

	vector dx(n);
	for(int i=0;i<n;i++) dx[i] = (1+std::abs(x[i]))*std::pow(2.0,-52.0/3);
 
	//AI generated

	vector phip(n), phim(n);
	for(int i=0;i<n;i++){
		vector xp = x; xp[i] += dx[i];
		vector xm = x; xm[i] -= dx[i];
		phip[i] = phi(xp);
		phim[i] = phi(xm);
	}

	for(int i=0;i<n;i++)
		H(i,i) = (phip[i] - 2*phix + phim[i]) / (dx[i]*dx[i]);

	for(int i=0;i<n;i++)
	for(int j=i+1;j<n;j++){
		vector xpp = x; xpp[i]+=dx[i]; xpp[j]+=dx[j];
		vector xpm = x; xpm[i]+=dx[i]; xpm[j]-=dx[j];
		vector xmp = x; xmp[i]-=dx[i]; xmp[j]+=dx[j];
		vector xmm = x; xmm[i]-=dx[i]; xmm[j]-=dx[j];
		double Hij = (phi(xpp)-phi(xpm)-phi(xmp)+phi(xmm)) / (4*dx[i]*dx[j]);
		H(i,j) = H(j,i) = Hij;
	}
...
std::tuple<vector,int> newton_minimize(std::function<double(const vector&)> phi, vector x, double acc, std::function<pp::vector(std::function<double(const vector&)> phi, vector x)> gradient, std::function<pp::matrix(std::function<double(const vector&)> phi, vector x)> hessian){
	int n = x.size();
	int steps = 0;
	for(; steps<1000; steps++){
		...
		//This was proposed by AI after failing for some starting points
		double hscale = 0;
		for(int i=0; i<n; i++) hscale = std::max(hscale, std::abs(H(i,i)));
		double lev = std::max(1e-6, 1e-6*hscale);
		for(int i=0; i<n; i++) H(i,i) += lev;
		qr QRH(H);
		vector dx = QRH.solve(-1.0*g);
		double lambda = 1.0;
	...
	return std::make_tuple(x,steps);
}


double wavelet(double u){
	return u*std::exp(-u*u);
}

double wavelet_deriv(double u){
	return (1-2*u*u)*std::exp(-u*u);
}

double wavelet_deriv2(double u){
	return (4*u*u*u - 6*u)*std::exp(-u*u);
}

double wavelet_integ(double u){
	return -0.5*std::exp(-u*u);
}
...
double ann::deriv(double x){
	...
	for(int i=0;i<n;i++){
		...
		double w = p[2*n+i];
		double u = (x-a)/b;
		sum += wavelet_deriv(u)*w/b;
	}
	return sum;
}

double ann::deriv2(double x){
	double sum = 0;
	for(int i=0;i<n;i++){
		double a = p[i];
		double b = p[n+i];
		double w = p[2*n+i];
		double u = (x-a)/b;
		sum += wavelet_deriv2(u)*w/(b*b);
	}
	return sum;
}

double ann::integ(double x){
	double sum = 0;
	for(int i=0;i<n;i++){
		double a = p[i];
		double b = p[n+i];
		...
		double u_x  = (x-a)/b;
		double u_0  = (0.0-a)/b;
		sum += w*b*(wavelet_integ(u_x) - wavelet_integ(u_0));
	}
	return sum;
}

//This is an AI generated improvement of my first attempt after I had problems reducing the Cost function
 
void ann::train(const pp::vector& xdata, const pp::vector& ydata, double acc){
	int N = xdata.size();
	if(ydata.size() != N) throw std::invalid_argument("ann::train: x,y size mismatch");
	int M = 3*n;
	ann* self = this;
	//AI also proposed this work around after problems with the derivatives
	const double lambda_width = 1e-3;
	auto make_gradC = [self,&xdata,&ydata,N,M,lambda_width](){
		return [self,&xdata,&ydata,N,M,lambda_width](const pp::vector& params) -> pp::vector {
			pp::vector saved = self->p;
			self->p = params;
			pp::vector gC(M);
			for(int k=0;k<N;k++){
				double r = self->response(xdata[k]) - ydata[k];
				for(int i=0;i<self->n;i++){
					double a = params[i];
					double b = params[self->n+i];
					double w = params[2*self->n+i];
					double u = (xdata[k]-a)/b;
					double fu  = wavelet(u);
					double dfu = wavelet_deriv(u);
					gC[i]            += 2*r*(-dfu*w/b);
					gC[self->n+i]    += 2*r*(-dfu*w*u/b);
					gC[2*self->n+i]  += 2*r*fu;
				}
			}
			for(int i=0;i<self->n;i++){
				double b = params[self->n+i];
				gC[self->n+i] += -2*lambda_width/(b*b*b);
			}
			self->p = saved;
			return gC;
		};
	};
	auto cost = [self,&xdata,&ydata,N](const pp::vector& params) -> double {
		pp::vector saved = self->p;
		self->p = params;
		double C = 0;
		for(int k=0;k<N;k++){
	...
	const int n_restarts = 200;
	std::mt19937 gen(0);
	std::uniform_real_distribution<double> ua(-1.0,1.0);
	std::uniform_real_distribution<double> ub(0.1,1.0);
	std::uniform_real_distribution<double> uw(-1.0,1.0);
 
	pp::vector best_p = p;
	double best_cost = cost(p);
	...
	for(int restart=0; restart<n_restarts; restart++){
		...
		auto gradC = make_gradC();
		pp::vector p_result;
		bool ok = true;
		try{
			p_result = pp::newton(gradC, p0, acc);
		} catch(const std::exception&){
			try{
				p_result = pp::broyden(gradC, p0, acc);
			} catch(const std::exception&){
				ok = false;
			}
		}
		...
		double c = cost(p_result);
		if(c < best_cost){
			best_cost = c;
			best_p = p_result;
		}
		if(best_cost < acc*acc) break;
	}
