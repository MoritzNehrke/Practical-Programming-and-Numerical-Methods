...
std::tuple<vector,vector> rkstep12(std::function<vector(double,vector)>,double x,vector y,double h);

std::tuple<pp::vector,pp::matrix> driver(std::function<vector(double,vector)> F,double a,double b,vector yinit,double h=0.125,double acc=0.01,double eps=0.01);

pp::vector default_dx(const pp::vector& x);

pp::matrix jacobian(std::function<pp::vector(const pp::vector&)> f, const pp::vector& x, const pp::vector& fx, const pp::vector& dx);

pp::vector newton(std::function<pp::vector(const pp::vector&)> f, pp::vector x, double acc = 1e-2, double alpha_min = 1e-3, int max_iter = 100, pp::vector dx = pp::vector());

pp::vector broyden(std::function<pp::vector(const pp::vector&)> f, pp::vector x, double acc = 1e-2, double alpha_min = 1e-3, int max_iter = 100, pp::vector dx = pp::vector());

pp::vector gradient_fd(std::function<double(const pp::vector&)> phi, pp::vector x);

pp::vector gradient_cd(std::function<double(const pp::vector&)> phi, pp::vector x);

pp::matrix hessian_cd(std::function<double(const pp::vector&)> phi, pp::vector x);

pp::matrix hessian_fd(std::function<double(const pp::vector&)> phi, pp::vector x);

std::tuple<pp::vector,int> newton_minimize(std::function<double(const pp::vector&)> phi, pp::vector x, double acc = 1e-3, std::function<pp::vector(std::function<double(const vector&)> phi, vector x)> gradient = gradient_fd, std::function<pp::matrix(std::function<double(const vector&)> phi, vector x)> hessian = hessian_fd);
...
struct ann{
    ...
    void train(const pp::vector& x, const pp::vector& y, double acc = 1e-3);
};
